...
# 这个类比较关键 生成3dobject标签
class Object3d(object):
    ''' 3d object label '''
#  kitti标签 
    def __init__(self, label_file_line):
        data = label_file_line.split(' ')  #['Pedestrian', '0.00', '0', '-0.20', '712.40', '143.00', '810.73', '307.92', '1.89', '0.48', '1.20', '1.84', '1.47', '8.41', ...]
        data[1:] = [float(x) for x in data[1:]]
        # extract label, truncation, occlusion
        self.type = data[0]  # 'Car', 'Pedestrian', ...
        self.cls_id = self.cls_type_to_id(self.type) #将类别转化为对应的标签
        self.truncation = data[1]  # truncated pixel ratio [0..1]  #截断
        self.occlusion = int(data[2])  # 遮挡0=visible, 1=partly occluded, 2=fully occluded, 3=unknown
        self.alpha = data[3]  # object observation angle [-pi..pi] 观察角
        # 在二维图情况下的目标框情况：左上角和右下角
        ...
        self.level_str = None
        self.level = self.get_obj_level() # kitti将数据划分了难易程度，
...
if __name__ == "__main__":
    result = read_label(r"D:\python_files4\Complex-YOLOv4-Pytorch-master\Complex-YOLOv4-Pytorch-master\dataset\kitti\training\label_2\000000.txt")
    print(result)