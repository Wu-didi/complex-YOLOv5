...
#import transformation, kitti_bev_utils, kitti_data_utils
...
class KittiDataset(Dataset):
    def __init__(self, dataset_dir, mode='train', lidar_transforms=None, aug_transforms=None, multiscale=False,
        ...
        self.img_size = cnf.BEV_WIDTH# 608
        self.min_size = self.img_size - 3 * 32 # 多尺度训练
    ...
    def load_img_with_targets(self, index):
        ...
        # 获取数据
        lidarData = self.get_lidar(sample_id)  # 点云数据
        objects = self.get_label(sample_id)  # label 标签文件，把一些数据给提取出来了
        calib = self.get_calib(sample_id)  # 坐标文件

        image = self.get_image(sample_id)

        labels, noObjectLabels = kitti_bev_utils.read_labels_for_bevbox(objects)
        # print("labels,noobject",labels,noObjectLabels)
        # 这里label返回 class的index 以及中心点、长宽高和yaw角
        # label : class_index,obj.t[0], obj.t[1], obj.t[2], obj.h, obj.w, obj.l, obj.ry
        # 将相机坐标系坐标linda
        ...
        #print("rgb",rgb_map.shape)  #[3,608,608]
    ...
    def get_lidar(self, idx):
        ...
        #np.fromfile（）可以用来读取这种bin的二进制格式文件
        # 最后返回的数据 shape (115384, 4) 以第一个为例
    ...
    def paint(self,pointcloud,semantic,calib):
        origin_pointcloud = copy.deepcopy(pointcloud)
        ### project all the pointcloud onto image
        ...
        return painted_pointcloud,origin_pointcloud,semantic_channel
...
if __name__ == "__main__":
    ...
    train_dataset = KittiDataset(r'D:\python_files4\Complex-YOLOv4-Pytorch-master\Complex-YOLOv4-Pytorch-master\dataset\kitti', mode='train', lidar_transforms=train_lidar_transforms,
                                aug_transforms=train_aug_transforms, multiscale=configs.multiscale_training,
                                num_samples=configs.num_samples, mosaic=False,
