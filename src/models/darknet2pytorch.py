...
from YOLOv5.models.common import BottleneckCSP,Conv,SPP,Focus
...
class Darknet(nn.Module):
    def __init__(self, cfgfile, use_giou_loss):
        ...
        #先解析模型构建的cfg文件
        self.blocks = parse_cfg(cfgfile) # 解析成为列表，由字典构成，每一个字典就是网络结构
    ...
    def forward(self, x, targets=None):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                continue
            elif block['type'] in ['Focus','BottleneckCSP','Conv','SPP','convolutional', 'maxpool', 'reorg', 'upsample', 'avgpool', 'softmax', 'connected']:
    ...
    def create_network(self, blocks):
        ...
        for block in blocks:
            if block['type'] == 'net':
                ...
                #将提取深度和宽度的因子提取出来
                if 'depth_multiple' in block:

                    depth_multiple = float(block['depth_multiple'])
                    width_multiple = float(block['width_multiple'])
            ...
            # 加入新的模块
            elif block['type'] == 'BottleneckCSP':
                filters = int(int(block['filters'])*width_multiple)
                num = int(block['num'])
                num_bot = max(round(num * depth_multiple), 1) 
                
                model = BottleneckCSP(prev_filters,filters,n=num_bot)
                models.append(model)
                ...
                k_s = int(block['size'])
                stride = int(block["stride"])
                model = Conv(prev_filters,filters,k=k_s,s=stride)
                models.append(model)
                ...
                filters = int(int(block['filters'])*width_multiple)             
                model = SPP(prev_filters,filters)
                models.append(model)
                ...
                model = Focus(prev_filters,filters,k=k_s)
                models.append(model)
