...
#   构建模型
    ...
    # 载入预训练部分的load weight from a checkpoint
    print("pre_weights_path:",configs.pretrained_path)
    ...
    # 断点续训部分resume weights of model from a checkpoint
    ...
    # Data Parallel
    # 因为并没有多GPU运算，经过这个函数之后，相当于转到gpu上
...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer):
   #训练之前的一些准备参数
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        # 文件，图像数据，labels
        # 关键在于如何理解这个图像数据或者说点云转之后的图像数据是如何构建的
